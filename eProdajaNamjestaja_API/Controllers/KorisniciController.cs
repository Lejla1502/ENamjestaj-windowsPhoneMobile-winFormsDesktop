...
namespace eProdajaNamjestaja_API.Controllers
{
    public class KorisniciController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/Korisnici/Provjera/{username}")]//--------------------------------------------------->provjeriti da li se koristi<-----------------------------------------
        public bool Provjera(string username )
        {
            Korisnici k=db.esp_Korisnici_SelectByUsername(username).FirstOrDefault();
            if (k != null)
                return true;
            else
                return false;
